...
/** 회원 전용 화면
 * @author 김찬희(cksgml0156@naver.com)
 */
	...
	public void memberMenu() {
		...
		do {
			try {
				...
				System.out.println("4. 비밀변호 변경(현재 비밀번호, 새 비밀번호, 새 비밀번호 확인)");
...
//		System.out.println("성별 : " + (Session.loginMember.getMemberGender().equals("M") ? "남" : "여"));
	...
	private void selectMemberList() {
		...
		try {
			...
			for(int i=0;i<memberList.size();i++) {
				System.out.printf("%d\t\t%s\t\t%s\t\t%s\n", i+1, memberList.get(i).getMemberId(), memberList.get(i).getMemberName(), memberList.get(i).getMemberGender());
			}
	...
	private void updatePassword() {
		...
		try {
			...
			int result = service.updatePassword(memberPw, changePw, Session.loginMember.getMemberNo());
