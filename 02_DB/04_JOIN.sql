...
-- ANSI 표준
SELECT EMP.EMP_ID , EMP.EMP_NAME , NVL(EMP.MANAGER_ID, '없음') 사수번호, NVL(MGR.EMP_NAME, '없음') 사수이름
FROM EMPLOYEE EMP
...
SELECT *
FROM EMPLOYEE
JOIN LOCATION ON(LOCAL_CODE = LOCATION_ID)  -- EMPLOYEE 와 LOCATION 이 합쳐졌는데 LOCATION_ID 가 없음
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
...
WHERE J.JOB_CODE = E.JOB_CODE AND DEPT_ID = DEPT_CODE AND LOCATION_ID = LOCAL_CODE AND JOB_NAME = '대리' AND LOCAL_NAME LIKE 'ASIA%';
...
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE)
WHERE EMP_NO LIKE '7%' AND EMP_NAME LIKE '전%' AND SUBSTR(EMP_NO, 8, 1) = '2';

SELECT EMP_NAME , EMP_NO , DEPT_TITLE, JOB_NAME
FROM EMPLOYEE, JOB, DEPARTMENT
WHERE EMPLOYEE.JOB_CODE = JOB.JOB_CODE AND DEPT_ID = DEPT_CODE AND EMP_NO LIKE '7%' AND EMP_NAME LIKE '전%';
...
WHERE E.JOB_CODE = J.JOB_CODE  AND DEPT_ID=DEPT_CODE AND EMP_NAME LIKE '%형%';
...
-- SELF JOIN 사용
SELECT E.EMP_NAME , E.DEPT_CODE , A.EMP_NAME 동료이름
