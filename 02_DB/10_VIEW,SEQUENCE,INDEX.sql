...
--   NULL 값이 삽입될 수 없음.
-- ORA-01400: NULL을 ("KH_KCH"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다

-- 데이터 무결성 : 중복 X, NULL X -> 신뢰도 높은 데이터
--> 대부분의 컬럼에 NOT NULL 제약조건 추가 되어 있음
...
-- SEQ_TEST_NO의 증가값을 1, 최대값을 155로 수정
 ...
 * 
 * ** 인덱스의 단점 **
 * - 데이터 변경(INSERT, UPDATE, DELETE) 작업 시 트리 구조에 변형이 일어남
...
WHERE TEST_ID = 'TEST500000'; -- 29 / 19 / 28 ms

-- 인덱스 사용 O 조회
SELECT * FROM TB_INDEX_TEST
WHERE TEST_NO = 500000;  -- 1 / 3 / 0 ms
...
-- 인덱스 생성 후 조회
SELECT * FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000'; -- 1 / 0 ms  빨라짐
...
SELECT * FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000';  -- 31 / 21 ms  다시 느려짐
...
SELECT DEPT_TITLE, COUNT(*), AVG(SALARY) FROM EMPLOYEE LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) GROUP BY DEPT_TITLE ORDER BY DEPT_TITLE;