...
CREATE USER kch_sample IDENTIFIED BY "sample1234";
										--> 대소문자 구분
...
-- 3. 접속 권한 부여
-- GRANT 권한[,권한,...] TO 계정명;
GRANT CREATE SESSION TO kch_sample;
...
-- 6. 테이블 생성 권한 부여
--    + 객체 생성 공간 할당
GRANT CREATE TABLE TO kch_sample;

ALTER USER kch_sample DEFAULT TABLESPACE SYSTEM
...
GRANT SELECT ON EMPLOYEE TO kch_sample;
...
-- 4. 선동일 삭제
DELETE FROM KH_KCH.EMPLOYEE WHERE EMP_ID = 200;
-- ORA-01031: 권한이 불충분합니다

------------------------------kh 계정으로 접속-----------------------------------
-- 5. sample 계정의 EMPLOYEE 테이블 조회 권한 회수
REVOKE SELECT ON EMPLOYEE FROM kch_sample;
...
--> sample 계정에 시퀀스, 프로시저, 클러스터, 인덱스타입, 연산자 타입
--                  트리거, 테이블 생성 권한 부여
...
GRANT CREATE SEQUENCE,
	...
	CREATE TYPE,
	CREATE TRIGGER,
	CREATE TABLE TO kch_sample;

--> ROLE을 이용해서 간단히
GRANT RESOURCE TO kch_sample;
REVOKE RESOURCE FROM kch_sample;

-- 사용자에게 부여된 시스템 권한 조회
SELECT * FROM DBA_SYS_PRIVS
WHERE GRANTEE = 'KCH_SAMPLE';
...
GRANT CONNECT, RESOURCE, CREATE VIEW TO kch_sample;
									 -- (계정명)
