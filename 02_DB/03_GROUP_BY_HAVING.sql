...
-- 0RA-00937 : 단일 그룹의 그룹 함수가 아닙니다.


SELECT DEPT_CODE, SUM(SALARY)  -- 3) 그룹별 SALARY 합계를 조회
FROM EMPLOYEE   -- 1) EMPLOYEE 테이블에서
GROUP BY DEPT_CODE HAVING DEPT_CODE = 'D9'  -- 2) DEPT_CODE 가 같은 행 끼리 그룹을 지어
...
-- * GRUOP BY 절 : 같은 값들이 여러 개 기록된 행을 하나의 그룹으로 묶음
...
-- 부서코드 오름차순으로 조회
SELECT DEPT_CODE ,SUM(SALARY), AVG(SALARY), COUNT(*), MIN(HIRE_DATE), MAX(HIRE_DATE)
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE;
...
-- 인원수 내림차순 정렬
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') 성별, FLOOR(AVG(SALARY)) "급여 평균", SUM(SALARY) "급여 합계", COUNT(*) "인원 수"
FROM EMPLOYEE
...
SELECT DEPT_CODE , FLOOR(AVG(SALARY))  -- 4) DEPT_CODE 와 급여 평균 조회
FROM EMPLOYEE  -- 1) EMPLOYEE 테이블에서 
WHERE DEPT_CODE IN('D5','D6')  -- 2) 부서코드가 D5, D6인 행만 추려서
GROUP BY DEPT_CODE  -- 3) 추려진 결과 내에서 부서코드 별로 그룹을 지어
ORDER BY DEPT_CODE;  -- 5) DEPT_CODE 오름차순 정렬
...
-- EMPLOYEE 테이블에서
-- 부서별 70년대생의 급여 평균이 300만 이상인 부서를 찾아
-- 부서코드, 평균 급여(소수점 내림) 을 부서코드 내림차순 조회
SELECT DEPT_CODE , FLOOR(AVG(SALARY)) "평균 급여"
FROM EMPLOYEE
--WHERE TO_DATE(SUBSTR(EMP_NO,0,2), 'RR') BETWEEN '1970-01-01' AND '1979-12-31'
--WHERE SUBSTR(EMP_NO,0,2) BETWEEN 70 AND 79
--WHERE SUBSTR(EMP_NO,0,2) LIKE '7%'
...
-- UNION ALL 확인
-- 부서코드가 'D5' 또는 급여 300만 초과인 사원 조회(중복O)
SELECT EMP_ID , EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION ALL
