...
-- <DISTINCT> (별개의, 따로따로)
--> 조회 시 지정된 컬럼의 중복 값을 한 번만 표시할 때 사용
...
SELECT *
FROM EMPLOYEE
WHERE SALARY >= 3000000 AND 5000000 >= SALARY;


-- 컬럼명 BETWEEN A AND B : 컬럼 값이 A이상 B 이하인 경우 TRUE
SELECT * 
FROM EMPLOYEE
WHERE SALARY BETWEEN 3700000 AND 5000000;
...
SELECT *
FROM EMPLOYEE
--WHERE SALARY < 3000000 OR 5000000 < SALARY;
...
SELECT
	CASE
		WHEN (SELECT COUNT(*) FROM EMPLOYEE WHERE EMP_ID = 300 ) = 0 THEN 0
		WHEN (SELECT COUNT(*) FROM EMPLOYEE WHERE EMP_ID = 300 AND ENT_YN = 'Y') = 1 THEN 1
		ELSE 2
	END "CHECK"
FROM DUAL;

SELECT DEPT_CODE, NVL(DEPT_TITLE, '부서없음') DEPT_TITLE, COUNT(*) 인원, FLOOR(AVG(SALARY)) 평균 FROM EMPLOYEE LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) GROUP BY DEPT_CODE, DEPT_TITLE ORDER BY DEPT_CODE;
