...
-- EMPLOYEE 테이블에서
-- 사원명, 이메일, 이메일 중 '@' 위치 조회
SELECT EMP_NAME 사원명, EMAIL 이메일, INSTR(EMAIL, '@') 
FROM EMPLOYEE;
...
-- 주어진 문자열의 앞|뒤|양쪽에 존재하는 지정된 문자를 제거
...
-- 2023-03-07 12:22:46.000, 2023-03-07 12:22:46.166 +0900
...
SELECT 1000000,
	TO_CHAR(1000000, 'L0,000,000'),  -- ￦1,000,000
	TO_CHAR(1000000, '$0,000,000'),  -- $1,000,000
FROM DUAL;

-- EMPLOYEE 테이블에서
-- 사번, 이름, 연봉 조회(\100,000,000 형식으로 조회)
SELECT EMP_ID 사번, EMP_NAME 이름, TO_CHAR(SALARY * 12, 'L999,999,999') 연봉
FROM EMPLOYEE;
...
SELECT SYSDATE, TO_CHAR(SYSDATE, 'AM HH:MI:SS'), TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS DY')  FROM DUAL;
...
/* 날짜 패턴 Y, R의 차이점*/
-- 년도를 짧게 해석하는 경우 
-- 50 미만 : Y, R 둘다 앞부분에 현재 세기를 적용(21C=2000년대)를 적용
...
-- NUVL2(컬럼명, NULL X인 경우 값, NULL O인 경우 값)

-- EMPLOYEE 테이블에서
-- 기존에 보너스를 받지 못했던 사원은 0.3으로 변경
-- 기존에 받았던 사원은 기존 보너스 + 0.2으로 변경
SELECT EMP_NAME, BONUS, NVL2(BONUS, BONUS + 0.3, 0.3) "변경된 BOUNS"
FROM EMPLOYEE;
...
SELECT EMP_NAME, JOB_CODE, SALARY 기존급여, SALARY + SALARY * DECODE(JOB_CODE, 'J7', 0.1, 'J6', 0.15, 'J5', 0.2, 0.05) 지급급여
FROM EMPLOYEE;
...
-- COUNT(컬럼명) : 지정된 컴럼의 값이 NULL인 경우를 제외한 행의 개수를 반환
-- COUNT(DISTINCT 컬럼명) 
...
SELECT COUNT(*) "남자 사원수" FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 8, 1) = '1';

SELECT COUNT(*) "여자 사원수" FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 8, 1) = '2';

SELECT COUNT(DECODE(SUBSTR(EMP_NO,8,1), '1', 1, NULL)) "남자 사원수", COUNT(DECODE(SUBSTR(EMP_NO,8,1), '2', 1, NULL)) "여자 사원수"
FROM EMPLOYEE;

SELECT SUM(DECODE(SUBSTR(EMP_NO,8,1), '1', 1, 0)) "남자 사원수",
	SUM(DECODE(SUBSTR(EMP_NO,8,1), '2', 1, 0)) "여자 사원수"
FROM EMPLOYEE;

-- * 서브쿼리를 이용한 방법 *
SELECT
	(SELECT COUNT(*) FROM EMPLOYEE 
	WHERE SUBSTR(EMP_NO,8,1) = '1') "남자 사원수",
	(SELECT COUNT(*) FROM EMPLOYEE 
	WHERE SUBSTR(EMP_NO,8,1) = '2') "여자 사원수"
FROM DUAL;
...
SELECT EMP_ID, EMP_NAME, ENT_YN, ENT_DATE FROM EMPLOYEE WHERE EMP_ID = 223;
