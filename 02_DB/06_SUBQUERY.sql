...
/*  서브쿼리 유형
    ...
    - 단일행 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 1개일 때 (행 1, 열 1)
    
    - 다중행 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 여러개일 때 (행 N, 열 1)
    
    - 다중열 서브쿼리 : 서브쿼리의 SELECT 절에 자열된 항목수가 여러개 일 때(행 1, 열 M)
    
    - 다중행 다중열 서브쿼리 : 조회 결과 행 수와 열 수가 여러개일 때 (행 N, 열 M)
...
WHERE e.JOB_CODE = j.JOB_CODE AND DEPT_CODE = DEPT_ID(+) AND SALARY >= (SELECT AVG(SALARY) FROM EMPLOYEE);
...
SELECT EMP_ID "사번" , EMP_NAME "이름" , JOB_NAME "직급" , DEPT_TITLE "부서", SALARY "급여", HIRE_DATE "입사일"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE EMP_ID = (SELECT /*ROWNUM,*/ EMP_ID FROM(SELECT EMP_ID FROM EMPLOYEE ORDER BY SALARY, HIRE_DATE)  -- 급여, 입사일을 기준으로 정렬
WHERE ROWNUM = 1);
--WHERE ;
...
-- 사수에 해당하는 직원(MANAGER_ID가 NULL)에 대해 조회 
...
SELECT EMP_ID , EMP_NAME "사수" , DEPT_TITLE, JOB_NAME, '사수' AS 구분
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
SELECT EMP_ID , EMP_NAME "사원" , DEPT_TITLE, JOB_NAME, '사원' AS 구분
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE EMP_ID NOT IN(SELECT EMP_ID FROM EMPLOYEE WHERE MANAGER_ID IS NULL);
...
EMP_ID NOT IN(SELECT DISTINCT MANAGER_ID FROM EMPLOYEE WHERE MANAGER_ID IS NOT NULL);
...
FROM EMPLOYEE e 
WHERE (DEPT_CODE,HIRE_DATE) = (SELECT DEPT_CODE,HIRE_DATE FROM EMPLOYEE WHERE EXTRACT (YEAR FROM HIRE_DATE)=2000);
...
--			--> 서브쿼리 조회 겨로가가 1행 이상이면 메인 쿼리 행을 결과에 포함
SELECT EMP_ID ,EMP_NAME ,DEPT_TITLE ,MANAGER_ID 
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)						
WHERE EXISTS (SELECT EMP_ID FROM EMPLOYEE SUB WHERE SUB.EMP_ID = MAIN.MANAGER_ID);				
						
						
SELECT EMP_ID ,EMP_NAME ,DEPT_TITLE ,MANAGER_ID 
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE MANAGER_ID IN (SELECT EMP_ID FROM EMPLOYEE);
...
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE (DEPT_CODE, HIRE_DATE) IN (SELECT DEPT_CODE, MIN(HIRE_DATE) FROM EMPLOYEE SUB  WHERE ENT_YN='N' GROUP BY DEPT_CODE)
ORDER BY HIRE_DATE;
...
WHERE HIRE_DATE
	= (SELECT MIN(HIRE_DATE)  FROM EMPLOYEE SUB 
	   WHERE ENT_YN = 'N' AND(SUB.DEPT_CODE = MAIN.DEPT_CODE -- NULL X
	   OR MAIN.DEPT_CODE IS NULL AND SUB.DEPT_CODE IS NULL) -- NULL인 경우
	  )
ORDER BY HIRE_DATE;
...
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE HIRE_DATE
	= (SELECT MIN(HIRE_DATE)  FROM EMPLOYEE SUB  WHERE ENT_YN = 'N' AND NVL(SUB.DEPT_CODE,'1') = NVL(MAIN.DEPT_CODE,'1') )
ORDER BY HIRE_DATE;
...
SELECT EMP_NAME, JOB_CODE, SALARY, (SELECT FLOOR(AVG(SALARY)) FROM EMPLOYEE SUB WHERE SUB.JOB_CODE = MAIN.JOB_CODE) 평균
FROM EMPLOYEE MAIN;
...
-- (스칼라 + 상관 쿼리)
SELECT EMP_ID , EMP_NAME , MANAGER_ID , NVL((SELECT EMP_NAME FROM EMPLOYEE SUB WHERE SUB.EMP_ID = MAIN.MANAGER_ID), '없음') 관리자명
FROM EMPLOYEE MAIN;

-- 셀프 조인
SELECT MAIN.EMP_ID, MAIN.EMP_NAME, MAIN.MANAGER_ID, NVL(SUB.EMP_NAME, '없음') 관리자명
FROM EMPLOYEE MAIN
...
--    반드시 조건 범위 내에 1 (1번 행)이 반드시 포함되어야 한다
...
NATURAL JOIN JOB
LEFT JOIN DEPARTMENT ON (DEPT_CODE=DEPT_ID)
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE FROM EMPLOYEE WHERE EMP_NAME = '노옹철') AND EMP_NAME != '노옹철';
...
WHERE HIRE_DATE IN (SELECT MIN(HIRE_DATE) FROM EMPLOYEE SUB 
					WHERE ENT_YN='N'AND (MAIN.DEPT_CODE = SUB.DEPT_CODE OR (MAIN.DEPT_CODE IS NULL AND SUB.DEPT_CODE IS NULL)) )
ORDER BY HIRE_DATE ;

-- 7. -------------------------------------------------------------------------
SELECT EMP_ID, EMP_NAME, JOB_NAME,  TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,6), 'RRMMDD'))/12) 나이, TO_CHAR((SALARY + SALARY * NVL(BONUS,0))*12, 'L999,999,999') 보너스포함연봉
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE)
WHERE TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD') IN (SELECT MAX(TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD')) FROM EMPLOYEE SUB GROUP BY JOB_CODE)
ORDER BY 나이 DESC;

SELECT EMP_ID, EMP_NAME, JOB_NAME,  TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,6), 'RRRRMMDD'))/12) 나이, TO_CHAR((SALARY + SALARY * NVL(BONUS,0))*12, 'L999,999,999') 보너스포함연봉
FROM EMPLOYEE MAIN
JOIN JOB ON(MAIN.JOB_CODE = JOB.JOB_CODE)
WHERE TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD') = (SELECT MAX(TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD')) FROM EMPLOYEE SUB WHERE MAIN.JOB_CODE = SUB.JOB_CODE)
ORDER BY 나이 DESC;
