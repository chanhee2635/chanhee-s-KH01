...
//	@Before("execution(* edu.kh.project..*Impl*.*(..))") // 타겟 수행 전
	...
	// 지정된 클래스.메서드에 작성된 @Pointcut() 어노테이션에 내용을 타겟으로 삼음
	
	@Order(1)  // 순서, 하나의 타겟에 대한 여러 advice 수행시 순서 지정
	...
	public void beforeLog(JoinPoint jp) {  // Advice(수행할 코드)
		...
		// [Start] : MemberServiceImpl - login()  처럼 보임
   ...
   public static String getRemoteAddr(HttpServletRequest request) {
        ...
        if (ip == null || ip.length() == 0 || "unknowSn".equalsIgnoreCase(ip)) { 
