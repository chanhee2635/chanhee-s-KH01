...
//	@PostMapping("/login")
	...
	public String logout(SessionStatus status, HttpSession session) {
		...
		// Spring
		// 1) HttpSession 을 이용한 경우
		//   -> HttpSession.invalidate()
		
		// 2) Model + @SessionAttributes 를 이용한 경우
		//   -> SessionStatus.setComplete()
		
		status.setComplete();
		
//		session.invalidate(); // 세션 무효화 -> 호환 안됨...
	...
	public String signUp() {
		...
		// -> ViewResolber가 prefix, suffix를 리턴 값 앞, 뒤에 붙임
	...
	public String signUp(Member inputMember
		...
		// RedirectAttributes ra : 리다이렉트 시 데이터를 request scope 로 전달하는 객체
...
//		inputMember.setMemberAddress(addr);
		// -> 클라이언트가 , 를 직접 입력하는 문제 발생
		
		// 만약 주소를 입력하지 않은 경우(,,) null 로 변경
...
//			path += "/member/signUp";  // 절대 경로
	...
	// @ExceptionHandler(Exception.class)  // 주석하면 아무것도 못행
