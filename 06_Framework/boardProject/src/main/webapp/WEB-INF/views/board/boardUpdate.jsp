...
<body>
    <main>
        ...
        <%-- /board2/1/2006/update?cp=1 (GET) 현재 위치 --%>
        <%-- /board2/1/2006/update (POST) 상대 경로 요청 --%>
            ...
            <c:forEach items="${board.imageList}" var="img">
                <c:choose>
                    <c:when test="${img.imageOrder==0}">
                        <c:set var="img0" value="${img.imagePath}${img.imageReName}"/>  <%-- /resources/images/board/2023051121211.jpg --%>
                    </c:when>

                    <c:when test="${img.imageOrder==1}">
                        <c:set var="img1" value="${img.imagePath}${img.imageReName}"/>  <%-- /resources/images/board/2023051121211.jpg --%>
                    </c:when>
                
                    <c:when test="${img.imageOrder==2}">
                        <c:set var="img2" value="${img.imagePath}${img.imageReName}"/>  <%-- /resources/images/board/2023051121211.jpg --%>
                    </c:when>
                
                    <c:when test="${img.imageOrder==3}">
                        <c:set var="img3" value="${img.imagePath}${img.imageReName}"/>  <%-- /resources/images/board/2023051121211.jpg --%>
                    </c:when>
                        
                    <c:when test="${img.imageOrder==4}">
                        <c:set var="img4" value="${img.imagePath}${img.imageReName}"/>  <%-- /resources/images/board/2023051121211.jpg --%>
                    </c:when>
                </c:choose>
