...
function clickCount(cnt){
    // 클릭할 때 마다 1씩 증가
    // 단, 20을 초과하면 다시 0으로 돌아가기
    cnt.innerText++; /* 똑똑이라 자동으로 형변환 */
    // cnt.innerText = Number(btn.innerText) + 1;
    if(cnt.innerText>20){
        cnt.innerText=0;
    }
}
...
document.querySelector("#button4").addEventListener("click", () => {
    ...
    printConsole(num => {return num*1000}); // 매개변수 1개 -> ()생략
    // 함수 정의 부분에 return 만 작성
    printConsole(num => num*10000);
...
button6.addEventListener("click", function(){
    console.log(this);
    // 이벤트 핸들로로 지정된 익명 함수의 this == 이벤트가 발생한 요소
    this.style.fontSize = "25px";
});

// 클릭된 요소의 테두리를 3px solid pink로 변경(화살표, e 사용 x, this 사용 o)
